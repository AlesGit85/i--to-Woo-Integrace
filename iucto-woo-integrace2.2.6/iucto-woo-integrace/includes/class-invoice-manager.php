...
/**
 * Invoice Manager třída - VERZE 2.1.10
 * 
 * OPRAVA: Odstraněno přidávání vat_account_id (stejně jako starý plugin).
 * Starý plugin od KK webdesign nepřidával vat_account_id a fungovalo to!
 * 
...
class IUcto_Woo_Invoice_Manager
{
    ...
    public function create_proforma_invoice($order)
    {
        ...
        $invoice_data = $this->build_invoice_payload($order, 'advance');
     ...
     * 
     * Všechny faktury (proforma i tax) obsahují stejné parametry:
     * - chart_account_id: ID účtu
     * - accountentrytype_id: ID typu zápisu
     * - vat_chart_id: ID účtu DPH (pouze pokud DPH > 0)
     * 
     * @param WC_Order $order Instance objednávky
     * @param string   $type  Typ faktury ('advance' nebo 'tax')
    ...
    private function build_invoice_items($order, $type = 'tax')
    {
        ...
        foreach ($order->get_items() as $item) {
            ...
            // Parametry pro účetnictví
            $item_data['accountentrytype_id'] = $accountentrytype_id;
            
            // Účet DPH pouze pokud má položka nenulovou sazbu
            if ($item_vat > 0) {
                $item_data['vat_chart_id'] = $vat_account_id;
            }
        ...
        foreach ($order->get_items('shipping') as $ship_item) {
            ...
            // Účet DPH pouze pokud má doprava nenulovou sazbu
            if ($shipping_vat > 0) {
                $ship_data['vat_chart_id'] = $vat_account_id;
            }
